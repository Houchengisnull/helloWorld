...
`FutureTask`类实现了`RunnableFuture`接口，**RunnableFuture 继承了 Runnable 接口和 Future 接口**，而`FutureTask`实现了`RunnableFuture`接口。所以它既可以 作为 Runnable 被线程执行，又可以作为Future`得到Callable`的返回值。
