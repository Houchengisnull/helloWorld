...
在使用`FutureTask`获取任务的执行结果时，众所周知，`FutureTask`的`get`方法是一个阻塞方法，必须**同步地**等待线程将`当前任务`完成后才能获取`下一个任务`的执行结果。

而在并发编程实际情况下，后面提交的任务也许先于前面提交的任务先完成。如此一来，用户必须按照同步的等待所有任务完毕后才能获取结果，导致系统响应时间延长。

`CompletionService`的实现时`ExecutorCompletionService`。`ExecutorCompletionService`将具体的计算任务交给`Executor`来完成，用`BlockingQueue`来保存任务的计算结果。

当提交一个任务到`ExecutorCompletionService`时，首先将任务包装成`QueueingFuture`。`QueueingFuture`是`FutureTask`的子类，改写`FutureTask`的`done()`方法，将`Executor`的计算结果放入`BlockingQueue`中。

而`CompletionService`则将`Executor`和`BlockingQueue`结合，对`ExecutorService`进行了包装，内部维护一个保存`Future`对象的`BlockingQueue`。只有当`Future`对象状态是结束的时候，才会加入到阻塞队列中。

所以，任务先完成可被先获取，***同时以减少不必要的等待时间***。
