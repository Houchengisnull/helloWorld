...
用更通俗易懂的中国话描述：**<font style='color:red;'>像使用自己的代码一样。</font>**

而`Stub`就是对程序员暴露的方法或者函数，在程序员调用远程接口时就像本地调用一样丝滑。

之所以叫`Stub`，个人猜测要这样理解：

> 不论是`client stub`还是`server stub`，都是真正的`implements`给`client`或者`server`的一个`stub(存根)`。
>
> 当(`sever`)收到请求后需要通过`stub`找到真正的`implements`。
