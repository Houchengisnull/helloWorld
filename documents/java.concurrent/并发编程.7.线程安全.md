...
针对这种情况，我们**在线程释放锁后**，令当前线程休眠随机时长，以错开申请锁的时间。
...
这是因为在`new SingleDcl()`这一步中，从逻辑来看实际分为三个步骤：

1. 在内存中分配空间；
2. 初始化对象；
3. 将对象指向分配的内存地址；

但是由于`CPU`的会对我们的代码进行优化——`指令重排序`。所以真正的执行顺序也许并不是我们笃定的那样。

有可能分配空间后，首先执行第三步将对象指向我们的空间。如果这个其他的线程将认为这个对象不为空，但是由于没有初始化对象，在调用时将抛出空指针异常。

为了解决这个问题，我们需要对`singleDcl`用`volatile`修饰，令这个对象在获取时防止指令重排序。

> - 2021-4-5
>
>   当我们需要对性能有所追求时，我们应该使用`双重检查锁定模式`。毕竟 `volatile`才是最轻量的同步机制，其次在使用单例模式时，更多的情况往往时`instance`已被初始化。
>
>   见`《Effective Java》第71条`。
...
> - 《Effective Java》
>
>   当`getInstance`首次调用，进程第一次读取`InstanceHolder`，致使它被初始化。

### 饿汉式

在声明对象的时候就`new`这个类的实例，因为在`JVM`中，对类的加载和类初始化，由虚拟机保证线程安全。
...
`《Effective Java》`中第77条，这里应该针对的是非单例模式的情况。

因为非单例模式的情况下，每个线程都会在使用时初始化，延迟占位显得多余且降低性能——执行了更多的代码。