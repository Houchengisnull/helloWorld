...
所有左子树的元素小于或者大于双亲结点的数据，所有右子树的元素大于或者小于双亲结点的数据。
...
代码见：`org.hc.learning.datastruct.tree.二叉树.AVLTree`

### 旋转

旋转是用来保持`AVL树`性质的技术。

只有在插入结点后，从插入结点到`root`的路径上的结点的平衡因子才可能改变。为了恢复`AVL Tree`的性质，从插入点开始访问直到树的根结点。在移动到根结点的过程中，需要寻找第一个不满足`AVL 树`的结点。从这个结点到根结点的路径上的每个结点都存在k>1的问题。

因此，如果修复这个结点的问题，那么通往根结点上其他所有结点都将满足`AVL 树`的性质。

假设结点X是必须要重新平衡的结点。因为任意结点最多有两个孩子结点，并且高度不平衡需要X结点的高度差为2。所以容易观测出以下四种情况可能导致违反`AVL Tree`性质：
...
通常，解决第一、第二种情况需要单旋转，而针对第三、第四中情况我们需要双旋转。

针对第一种情况，我们需要进行`左左旋转`。

`左左旋转`即发现`X结点`后，将其左子结点移到X结点的位置，将其左子树的右子树放在`X结点`的左子树上，`X结点`则放在其左子结点的右子树上。这样便完成了左左旋转。

`左右旋转`为双旋转，首先对`X结点`的左结点进行`右右旋转`，再对`X结点`进行`左左旋转`。

> 在实现时，需要每次插入及旋转完成后，再对旋转的结点进行高度计算。
